...
class ModelTrainer:
    ...
    def _build_model(self):
        if self.config.model_type=="RandomForest":
            if self.config.task_type=="classification":
                base = RandomForestClassifier(n_estimators=200,random_state=self.config.random_state,n_jobs=-1)
            else:
                base = RandomForestRegressor(n_estimators=200,random_state=self.config.random_state,n_jobs=-1)
        else: raise ValueError(f"Unsupported model type {self.config.model_type}")
    ...
    def train(self,X: pd.DataFrame, y: pd.Series) -> Tuple[Any, Dict[str,float]]:
        X_train,X_test,y_train,y_test = train_test_split(X,y,test_size=self.config.test_size,
                                                        shuffle=self.config.shuffle,random_state=self.config.random_state)
        ...
        if self.config.task_type=="classification":
            ...
            metrics={"mse":float(mse),"rmse":float(np.sqrt(mse)),"r2":float(r2_score(y_test,y_pred))}
